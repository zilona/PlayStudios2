...
Each video is shown on the home page for 121 days. However, for some videos there weren't views every day. To deal with this I added the value *zero* under the 'count' to the missing dates of the relevant videos.  
...
However, the 'K-means' madel didn't consider the importance of the regressions slope (the variable that represent the stability of the trend of viewing the videos).
...
## Weighting of variables count of views and regressions slope.
Looking at the 
...
```{r plot the clustering, eval=FALSE, include=FALSE}
plot(analized_data$`count of views`,analized_data$`regression slope`)
points(analized_data$`count of views`[analized_data$rank_after_Weighting==1],analized_data$`regression slope`[analized_data$rank_after_Weighting==1],col = 1,pch = 16)
points(analized_data$`count of views`[analized_data$rank_after_Weighting==2],analized_data$`regression slope`[analized_data$rank_after_Weighting==2],col = 2,pch = 16)
points(analized_data$`count of views`[analized_data$rank_after_Weighting==3],analized_data$`regression slope`[analized_data$rank_after_Weighting==3],col = 3,pch = 16)



plot(analized_data$`count of views`,analized_data$`regression slope`)
(cl <- kmeans(analized_data[,3:2], 3))
plot(analized_data[,3:2], col = cl$cluster)
points(cl$centers, col = 1:3, pch = 8)
abline(v = 12518.939,col = "red",lty = 3)
abline(a = 0,b=-1,col = "blue",lty = 1)


a <- analized_data[order(-analized_data$`mean of views`,analized_data$`regression slope`,analized_data$`count of views`),]


# pca <- prcomp(analized_data, scale = T)
# autoplot(kmeans(analized_data, 3), data = a)
# autoplot(prcomp(analized_data[,2:3]))
# (cl <- kmeans(analized_data[,2:3], 3))
# plot(analized_data[,2:3], col = cl$cluster)
# points(cl$centers, col = 1:3, pch = 8)
```
...
```{r load video features, include=FALSE}
...
ggplot(merged_data,aes(video_quality,video_length,col = as.factor(rank_after_Weighting)))+
  geom_point(size = 3)+
  scale_colour_discrete(name  ="Caterory",
                            breaks=c("1", "2","3"),
                            labels=c("Hot videos", "Stable and Popular","Everything else"))+
...
ggplot(video_data,aes(video_data$video_length,video_data$count,col = video_data$video_language))+
   geom_point()
